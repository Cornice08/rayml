...
let () = 
  ...
  in 
  let left_wall = Plane.{transform; material} in
  let transform = Matrix.sequence [
    Matrix.translation 0. 0. 5.;
    Matrix.rotation_y ((Float.pi) /. 4.);
    Matrix.rotation_x (Float.pi /. 2.);
  ...
  in 
  let right_wall = Plane.{transform; material} in
  ...
  let middle = { middle with material; transform = Matrix.sequence [
  ...
  let right = { right with material; transform = Matrix.sequence [
  ...
  let left = { left with material; transform = Matrix.sequence [
  ...
  let camera = { camera with transform = Matrix.view_transform from to_ up} in 
